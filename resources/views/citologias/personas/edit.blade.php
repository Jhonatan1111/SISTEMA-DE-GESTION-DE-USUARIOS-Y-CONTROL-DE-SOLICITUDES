<x-app-layout>
    ...
    <div class="max-w-5xl mx-auto px-4 py-6">
        ...
        <form action="{{ route('citologias.personas.update', $citologia->ncitologia) }}" method="POST" class="space-y-6">
            ...
            <input type="hidden" name="tipo" id="tipo_seleccionado" value="{{ old('tipo', $citologia->tipo) }}">
            <input type="hidden" name="ncitologia" id="ncitologia_nuevo" value="{{ $citologia->ncitologia }}">
            <input type="hidden" name="lista_id" id="lista_id_hidden" value="{{ old('lista_id', $citologia->lista_id) }}">
            ...
            <div class="bg-gradient-to-r from-blue-50 via-white to-blue-50 p-6 rounded-2xl shadow-xl border border-blue-200 transition-transform hover:-translate-y-1 hover:shadow-2xl">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="date" id="fecha_recibida" name="fecha_recibida" value="{{ old('fecha_recibida', $citologia->fecha_recibida->format('Y-m-d')) }}" max="{{ date('Y-m-d') }}" class="w-full px-4 py-2 border-2 border-blue-300 rounded-lg focus:ring-2 focus:ring-blue-400 focus:border-blue-500 transition-all" required>
                    ...
                    <div>
                        ...
                        <select id="paciente_id" name="paciente_id" class="w-full px-4 py-2 border-2 border-blue-300 rounded-lg focus:ring-2 focus:ring-blue-400 focus:border-blue-500 transition-all" required>
                            @foreach($pacientes as $paciente)
                            <option value="{{ $paciente->id }}" {{ old('paciente_id', $citologia->paciente_id) == $paciente->id ? 'selected' : '' }}>
                    ...
                    <div id="campo-remitente" class="md:col-span-2" style="display: none;">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label for="remitente_especial" class="block text-sm font-semibold text-gray-700 mb-1">Remitente Especial <span class="text-red-500">*</span></label>
                                <input type="text" id="remitente_especial" name="remitente_especial" value="{{ old('remitente_especial', $citologia->remitente_especial) }}" placeholder="Nombre del remitente..." class="w-full px-4 py-2 border-2 border-orange-300 rounded-lg focus:ring-2 focus:ring-orange-400 focus:border-orange-500 transition-all">
            ...
            <div class="bg-gradient-to-r from-green-50 via-white to-green-50 p-6 rounded-2xl shadow-xl border border-green-200 transition-transform hover:-translate-y-1 hover:shadow-2xl">
                ...
                <div class="space-y-4">
                    <!-- Descripción -->
                    <div>
                        <label for="descripcion" class="block text-sm font-semibold text-gray-700 mb-1">Descripción <span class="text-red-500">*</span></label>
                        <textarea id="descripcion" name="descripcion" rows="4" class="w-full px-4 py-2 border-2 border-green-300 rounded-lg focus:ring-2 focus:ring-green-400 focus:border-green-500 transition-all" placeholder="Descripción general de la muestra..." required>{{ old('descripcion', $citologia->descripcion) }}</textarea>
                        @error('descripcion')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
