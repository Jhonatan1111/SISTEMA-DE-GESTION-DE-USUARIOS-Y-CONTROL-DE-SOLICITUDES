...
<body>
    ...
    <div class="info">
        <div class="info-item">
            <span class="info-label">Fecha de generación:</span> {{ $fecha }}<br>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    <span class="badge badge-{{ $biopsia->tipo }}">{{ ucfirst($biopsia->tipo) }}</span>
                </td>
                <td>
                ...
                <td>
                    ...
                    <small>{{ $biopsia->mascota->especie }} - {{ $biopsia->mascota->dueno ?? $biopsia->mascota->propietario }}</small>
                    @endif
                </td>
                <td>Dr. {{ $biopsia->doctor->nombre }} {{ $biopsia->doctor->apellido }}</td>
                <td>{{ Str::limit($biopsia->diagnostico_clinico, 60) }}</td>
                <td>
                    <span class="badge badge-{{ $biopsia->estado ? 'activa' : 'inactiva' }}">{{ $biopsia->estado ? 'Activa' : 'Inactiva' }}</span>
                </td>
            </tr>
            @empty
            <tr>
                <td colspan="8" class="no-data">No se encontraron biopsias con los filtros aplicados</td>
            </tr>
            @endforelse
        </tbody>
    </table>
